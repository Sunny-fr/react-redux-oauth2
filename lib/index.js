...
var actions = exports.actions = {
    ...
    load_user: function load_user(user, token) {
        return {
            type: 'OAUTH_LOAD_USER',
            payload: { user: user, token: token }
        };
    },
    load_token: function load_token(token) {
        return {
    ...
    sync_user: function sync_user(token, cb) {
        return function (dispatch) {
            fetch_token(token).then(function (res) {
...
var reducer = exports.reducer = {
    oauth: function oauth() {
        ...
        switch (actions.type) {
            ...
            case 'OAUTH_ERROR':
                return _extends({}, state, { authenticating: false, user: null, error: actions.payload });
                break;
            case 'OAUTH_LOAD_USER':
                return _extends({}, state, { authenticating: false, user: actions.payload.user, token: actions.payload.token, error: null });
                break;
            case 'OAUTH_LOAD_TOKEN':
                return _extends({}, state, { authenticating: false, token: actions.payload.token, error: null });
                break;
            case 'OAUTH_SAVE_TOKEN':
                return _extends({}, state, { authenticating: false, token: actions.payload, error: null });
                break;
            case 'OAUTH_SIGNOUT':
    ...
    return (0, _reactRedux.connect)(function (state) {
        ...
        _createClass(_class3, [{
            ...
            value: function handleClick(e, provider) {
                var url = '' + config.url + config.providers[provider] + '?client=' + config.clientID;
